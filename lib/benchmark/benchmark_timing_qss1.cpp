...
make_neuron_izhikevich(irt::simulation* sim, int i, double quantum, double a, double b, double c, double d, double I, double vini ) noexcept
{
  ...
  struct neuron_izhikevich neuron_model = { sim->qss1_wsum_2_models.get_id(sum_a),
                                            sim->qss1_wsum_2_models.get_id(sum_b),
                                            sim->qss1_wsum_2_models.get_id(sum_d),
                                            sim->qss1_wsum_4_models.get_id(sum_c),
                                            sim->qss1_multiplier_models.get_id(product),
                                            ...
                                            sim->qss1_cross_models.get_id(cross), 
                                            sim->qss1_cross_models.get_id(cross2),                                                                                                                                     
...
make_neuron_lif(irt::simulation* sim, int i, double quantum, double tau) noexcept
{
  ...
  struct neuron_lif neuron_model = {sim->qss1_wsum_2_models.get_id(sum_lif),
                              ...
                              irt::output_port_id presynaptic,irt::output_port_id postsynaptic,double quantum)
{
  ...
  auto& int_pre = sim->qss1_integrator_models.alloc();
  auto& sum_pre = sim->qss1_wsum_2_models.alloc();
  auto& mult_pre = sim->qss1_wsum_2_models.alloc();
  auto& cross_pre = sim->qss1_cross_models.alloc();

  auto& int_post = sim->qss1_integrator_models.alloc();
  auto& sum_post = sim->qss1_wsum_2_models.alloc();
  auto& mult_post = sim->qss1_wsum_2_models.alloc();
  auto& cross_post = sim->qss1_cross_models.alloc();
  ...
  !expect(irt::is_success(sim->alloc(int_pre, sim->qss1_integrator_models.get_id(int_pre))));
  !expect(irt::is_success(sim->alloc(sum_pre, sim->qss1_wsum_2_models.get_id(sum_pre))));
  !expect(irt::is_success(sim->alloc(mult_pre, sim->qss1_wsum_2_models.get_id(mult_pre))));
  !expect(irt::is_success(sim->alloc(cross_pre, sim->qss1_cross_models.get_id(cross_pre))));

  !expect(irt::is_success(sim->alloc(int_post, sim->qss1_integrator_models.get_id(int_post))));
  !expect(irt::is_success(sim->alloc(sum_post, sim->qss1_wsum_2_models.get_id(sum_post))));
  !expect(irt::is_success(sim->alloc(mult_post, sim->qss1_wsum_2_models.get_id(mult_post))));
  !expect(irt::is_success(sim->alloc(cross_post, sim->qss1_cross_models.get_id(cross_post))));
  ...
  struct synapse synapse_model = {sim->qss1_wsum_2_models.get_id(sum_pre),
                                  sim->qss1_wsum_2_models.get_id(mult_pre),
                                  sim->qss1_integrator_models.get_id(int_pre),
                                  sim->qss1_cross_models.get_id(cross_pre),  

                                  sim->qss1_wsum_2_models.get_id(sum_post),
                                  sim->qss1_wsum_2_models.get_id(mult_post),
                                  sim->qss1_integrator_models.get_id(int_post),
                                  sim->qss1_cross_models.get_id(cross_post),  
...
void network(neuron_type T, std::string matrix_name,int n_dim, int m_dim, double simulation_duration, double quantum_synapse, double quantum_neuron, double spike_rate)
{
 using namespace boost::ut;
      ...
      else if (matrix_name == "empty")
      {
        double* I = new double[n_dim*n_dim];
        double* J = new double[n_dim*n_dim];
        double* NNZ = new double[n_dim*n_dim];

        matrix = {n_dim,n_dim,0,I,J,NNZ};
      }
      ...
      expect(irt::is_success(sim.qss1_integrator_models.init(base + 
        two * N * N + N)));
      expect(irt::is_success(sim.qss1_wsum_2_models.init(base + two*(two * N * N + N))));
      expect(irt::is_success(sim.qss1_cross_models.init(base + two * N * N + N)));
      expect(irt::is_success(sim.qss1_wsum_4_models.init(base + N )));
      expect(irt::is_success(sim.qss1_multiplier_models.init(base + N )));
...
BENCHMARK_P_INSTANCE(Network, matrix,(gener,"empty",100,100,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix, (gener,"chesapeake.mtx",39,39,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix, (gener,"celegansneural.mtx",297,297,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix, (gener,"west0655.mtx",655,655,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix, (gener,"jpwh_991.mtx",991,991,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix,(gener,"fully connected",10,10,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix,(gener,"bipartite fully connected",10,10,500,1e-2,0.1,25));

BENCHMARK_P_INSTANCE(Network, matrix,(leaky_int_fire,"empty",100,100,500,1e-2,0.1,250));
BENCHMARK_P_INSTANCE(Network, matrix,(leaky_int_fire,"chesapeake.mtx",39,39,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix, (leaky_int_fire,"celegansneural.mtx",297,297,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix, (leaky_int_fire,"west0655.mtx",655,655,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix, (leaky_int_fire,"jpwh_991.mtx",991,991,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix,(leaky_int_fire,"fully connected",10,10,500,1e-2,0.1,25));
BENCHMARK_P_INSTANCE(Network, matrix,(leaky_int_fire,"bipartite fully connected",10,10,500,1e-2,0.1,25));

BENCHMARK_P_INSTANCE(Network, matrix,(izhikevich,"empty",100,100,500,1e-2,0.1,0.02));
BENCHMARK_P_INSTANCE(Network, matrix, (izhikevich,"chesapeake.mtx",39,39,500,1e-2,0.1,0.02));
BENCHMARK_P_INSTANCE(Network, matrix, (izhikevich,"celegansneural.mtx",297,297,500,1e-2,0.1,0.02));
BENCHMARK_P_INSTANCE(Network, matrix, (izhikevich,"west0655.mtx",655,655,500,1e-2,0.1,0.02));
BENCHMARK_P_INSTANCE(Network, matrix, (izhikevich,"jpwh_991.mtx",991,991,500,1e-2,0.1,0.02));
BENCHMARK_P_INSTANCE(Network, matrix,(izhikevich,"fully connected",10,10,500,1e-2,0.1,0.02));
BENCHMARK_P_INSTANCE(Network, matrix,(izhikevich,"bipartite fully connected",10,10,500,1e-2,0.1,0.02));
